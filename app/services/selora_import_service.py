...
class SeloraImportService:
    ...
    def send(
        ...
        *,
        ...
        checkpoint_service = SeloraUploadCheckpointService()
        checkpoint = checkpoint_service.load(
            draft_id=draft.id,
            workspace_id=workspace_id,
        )
        total_uploads = len(pending_uploads)
        ...
        for index, pending in enumerate(pending_uploads, start=1):
            asset_key = checkpoint_service.asset_key(
            ...
            if checkpoint_service.is_uploaded(
                ...
                logger.info(
                    (
                        "Skipping already uploaded asset %s/%s: "
            ...
            checkpoint_service.update_progress(
                ...
                total=total_uploads,
                current=index,
                current_media_id=pending.instagram_media_id,
                current_asset_type=pending.asset_type,
            )

            size_bytes = pending.file_path.stat().st_size
            request_id = f"{draft.id}:asset:{index}"

            logger.info(
                (
                    "Uploading asset %s/%s: media=%s type=%s position=%s "
                    "size=%s bytes"
                ),
                index,
                total_uploads,
            ...
            try:
                upload_result = self.client.upload_workspace_asset(
                    workspace_id=workspace_id,
                ...
                logger.error(
                    (
                        "Asset upload failed %s/%s: media=%s type=%s "
                        "position=%s status=%s code=%s request_id=%s "
                        "retryable=%s content_type=%s response=%s"
                    ),
                    index,
                    total_uploads,
                    pending.instagram_media_id,
                    ...
                    getattr(exc, "request_id", request_id),
                    getattr(exc, "retryable", False),
                    getattr(exc, "content_type", ""),
                    getattr(exc, "response_preview", ""),
                )
                raise
            ...
            logger.info(
                (
                    "Asset uploaded successfully %s/%s: media=%s type=%s "
    ...
    def _prepare_selected_assets(
        ...
        *,
        ...
        logger.info(
            (
                "Preparing selected Selora assets: draft=%s selected_assets=%s "
                "storage_root=%s"
            ),
            draft.id,
            len(selected_assets),
        ...
        if missing_assets:
            logger.info(
                (
                    "Persisting only missing selected assets: "
                    "draft=%s missing=%s selected=%s"
                ),
                draft.id,
                len(missing_assets),
                len(selected_assets),
        ...
        derivative_service = SeloraMediaDerivativeService()
        ...
        for asset in selected_assets:
            ...
            logger.info(
                (
                    "Preparing Selora asset: draft=%s media_id=%s type=%s "
                    "position=%s file=%s bytes=%s"
                ),
                draft.id,
            ...
            source_sha256 = asset.local_sha256 or self._sha256(file_path)

            try:
                prepared = derivative_service.prepare_webp(
                    source_path=file_path,
                    source_sha256=source_sha256,
                )
            except SeloraMediaDerivativeError as exc:
                raise SeloraPayloadMappingError(
                    "Preparing selected Instagram image for Selora failed."
                ) from exc

            logger.info(
                (
                    "Selora derivative ready: draft=%s media_id=%s type=%s "
                    "position=%s file=%s bytes=%s sha256=%s"
                ),
                draft.id,
                asset.media.media_id,
                asset.asset_type,
